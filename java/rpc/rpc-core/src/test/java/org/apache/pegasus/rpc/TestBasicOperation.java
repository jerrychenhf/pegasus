...
public class TestBasicOperation {
  ...
  public void roundTripTicket() throws Exception {
    final Ticket ticket = new Ticket(new byte[]{0, 1, 2, 3, 4, 5}, new byte[0], new byte[0], new int[0]);
  ...
  public void roundTripInfo() throws Exception {
    ...
    final FlightInfo info2 = new FlightInfo(schema, FlightDescriptor.command(new byte[2]),
        Collections.singletonList(new FlightEndpoint(
            new Ticket(new byte[10], new byte[10], new byte[10], new int[]{1,2}), Location.forGrpcDomainSocket("/tmp/test.sock"))), 200, 500);
    final FlightInfo info3 = new FlightInfo(schema, FlightDescriptor.path("a", "b"),
        Arrays.asList(new FlightEndpoint(
                new Ticket(new byte[10], new byte[10], new byte[10], new int[]{1,2}), Location.forGrpcDomainSocket("/tmp/test.sock")),
            new FlightEndpoint(
                new Ticket(new byte[10], new byte[10], new byte[10], new int[]{1,2}), Location.forGrpcDomainSocket("/tmp/test.sock"),
  ...
  public void getStream() throws Exception {
    test(c -> {
      FlightStream stream = c.getStream(new Ticket(new byte[0], new byte[0], new byte[0], new int[0]));
