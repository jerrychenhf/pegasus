...
import org.apache.pegasus.rpc.{FlightDescriptor, FlightInfo, Location}
import org.apache.spark.sql.execution.datasources.v2.pegasus.{DataReader, GrpcDataReader, LocalDataReader, PegasusClientFactory}

import scala.collection.JavaConverters._

object LocalReaderFileBatchTesting {

    def main(args: Array[String]): Unit = {

      val value = System.getProperty("java.library.path");
      ...
      val localRead = true
      if(localRead) {
        dataReader = new LocalDataReader(workerClient, ticket)
      } else {
        dataReader = new GrpcDataReader(workerClient, ticket)
      }
...
//          bodyarrowBuf.getReferenceManager.release
          ...
          val chunkBytes = new Array[Byte](bodyBuf.capacity())
          bodyBuf.capacity()
          bodyBuf.get(chunkBytes, 0, bodyBuf.capacity())
          ...
          System.out.println("bodyBuf.position(): " + bodyBuf.position())
          System.out.println("bodyBuf.get(): " + bodyBuf.get(10))
