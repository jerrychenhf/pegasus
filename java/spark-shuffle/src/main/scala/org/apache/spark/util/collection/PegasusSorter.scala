...
import org.apache.spark.shuffle.pegasus.{PegasusBlockObjectWriter, PegasusShuffleBlockResolver, PegasusShuffleManager, PegasusShuffleUtils}
...
/**
  ...
  */
private[spark] class PegasusSorter[K, V, C](
    context: TaskContext,
    ...
    extends Spillable[PegasusWritablePartitionedPairCollection[K, C]](context.taskMemoryManager())
        with Logging {
  ...
  @volatile private var map = new PegasusPartitionedAppendOnlyMap[K, C]
  @volatile private var buffer = new PegasusPartitionedPairBuffer[K, C]
  ...
  private def maybeSpillCollection(usingMap: Boolean): Unit = {
    ...
    if (usingMap) {
      ...
      if (maybeSpill(map, estimatedSize)) {
        map = new PegasusPartitionedAppendOnlyMap[K, C]
      }
    } else {
      estimatedSize = buffer.estimateSize()
      if (maybeSpill(buffer, estimatedSize)) {
        buffer = new PegasusPartitionedPairBuffer[K, C]
      }
    }
  ...
  /**
    ...
    */
  override protected[this] def spill(collection: PegasusWritablePartitionedPairCollection[K, C]): Unit = {
  ...
  /**
    ...
    */
  private[this] def spillMemoryIteratorToDisk(inMemoryIterator: PegasusWritablePartitionedIterator)
  : SpilledFile = {
    // Because these files may be read during shuffle, their compression must be controlled by
    // spark.shuffle.compress instead of spark.shuffle.spill.compress, so we need to use
    // createTempShuffleBlock here; see SPARK-3426 for more context.
    val (blockId, path) = resolver.createTempShuffleBlock()

    // These variables are reset after each flush
    var objectsWritten: Long = 0
    val spillMetrics: ShuffleWriteMetrics = new ShuffleWriteMetrics
    val writer: PegasusBlockObjectWriter =
      PegasusShuffleUtils.getPegasusWriter(
        blockId, path, serializerManager, serInstance, fileBufferSize, spillMetrics)
    ...
    try {
      ...
      if (success) {
        ...
        if (ps.existsPartition(path)) {
          if (!ps.deletePartition(path)) {
    ...
    SpilledFile(path, blockId, batchSizes.toArray, elementsPerPartition)
  }
  ...
  private[this] class SpillReader(spill: SpilledFile) {
    ...
    var dataStream: PegasusDataInputStream = null
    var deserializeStream = nextBatchStream()  // Also sets dataStream
    ...
    def nextBatchStream(): DeserializationStream = {
      ...
      if (batchId < batchOffsets.length - 1) {
        ...
        dataStream = ps.openPartition(spill.path, batchId, batchId+1)
        batchId += 1
        ...
        val bufferedStream = new BufferedInputStream(ByteStreams.limit(dataStream, end - start))
    ...
    def cleanup() {
      ...
      // TO DO: any clean up for pegasus here?
  ...
  /**
    * NOTE This basically turns a memoryIterator to a spillable(to Pegasus in memory storage) iterator
    *
    * Returns a destructive iterator for iterating over the entries of this map.
    * If this iterator is forced spill to Pegasus in memory storage to release memory when there is not enough
  ...
  def partitionedIterator: Iterator[(Int, Iterator[Product2[K, C]])] = {
    ...
    val collection: PegasusWritablePartitionedPairCollection[K, C] = if (usingMap) map else buffer
    if (spills.isEmpty) {
  ...
  /**
    ...
    */
  def writePartitionedOutput(
      blockId: BlockId,
      output: PegasusPath): Array[Long] = {

    // Track location of each range in the output file
    val lengths = new Array[Long](numPartitions)
    val writer = PegasusShuffleUtils.getPegasusWriter(
      blockId, output, serializerManager, serInstance, fileBufferSize,
  ...
  def stop(): Unit = {
    spills.foreach(s => ps.deletePartition(s.path))
    spills.clear()
    forceSpillFiles.foreach(s => ps.deletePartition(s.path))
  ...
  private[this] class SpillableIterator(var upstream: Iterator[((Int, K), C)])
    ...
    def spill(): Boolean = SPILL_LOCK.synchronized {
      if (hasSpilled) {
        ...
        val inMemoryIterator = new PegasusWritablePartitionedIterator {
