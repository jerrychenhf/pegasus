...
import org.apache.spark.shuffle.pegasus.storage.{PegasusShuffleOutputManagedBuffer}
 ...
 */
private[spark]
final class PegasusShuffleBlockIterator(
    context: TaskContext,
  ...
  import PegasusShuffleBlockIterator._
   ...
   */
  private[this] val results = new LinkedBlockingQueue[PegasusFetchResult]

  /**
   * Current [[PegasusFetchResult]] being processed. We track this so we can release
   * the current buffer in case of a runtime exception when processing the current buffer.
   */
  @volatile private[this] var currentResult: SuccessPegasusFetchResult = null

  /**
   * Queue of fetch requests to issue; we'll pull requests off this gradually to make sure that
   * the number of bytes in flight is limited to maxBytesInFlight.
   */
  private[this] val fetchRequests = new Queue[PegasusFetchRequest]
   ...
   */
  private[this] val deferredFetchRequests = new HashMap[BlockManagerId, Queue[PegasusFetchRequest]]()
  ...
  private[this] val onCompleteCallback = new PegasusShuffleCompletionListener(this)
  ...
  private[pegasus] def cleanup() {
    ...
    while (iter.hasNext) {
      ...
      result match {
        case SuccessPegasusFetchResult(_, _, _, _, buf, _) =>
  ...
  private[this] def sendRequest(req: PegasusFetchRequest) {
    ...
    val blockFetchingListener = new BlockFetchingListener {
      override def onBlockFetchSuccess(blockId: String, buf: ManagedBuffer): Unit = {
        val casted = buf.asInstanceOf[PegasusShuffleOutputManagedBuffer]
        val res = Future {
        ...
        } (PegasusShuffleBlockIterator.executionContext)
        res.onComplete {
          case Success(_) =>
            PegasusShuffleBlockIterator.this.synchronized {
              // Only add the buffer to results queue if the iterator is not zombie,
              // i.e. cleanup() has not been called yet.
              if (!isZombie) {
                remainingBlocks -= blockId
                results.put(SuccessPegasusFetchResult(
                  BlockId(blockId),
                  infoMap(blockId)._2,
                  address,
                  infoMap(blockId)._1,
          ...
          case Failure(e) =>
            results.put(FailurePegasusFetchResult(BlockId(blockId), infoMap(blockId)._2, address, e))
  ...
  // For pegasus shuffling, all blocks are remote, so this actually resembles PegasusFetchRequests
  ...
  // this project.
  private[this] def splitLocalRemoteBlocks(): ArrayBuffer[PegasusFetchRequest] = {
    ...
    val remoteRequests = new ArrayBuffer[PegasusFetchRequest]
  ...
  private def createFetchRequests(
      ...
      isLast: Boolean,
      collectedRemoteRequests: ArrayBuffer[PegasusFetchRequest]): Seq[FetchBlockInfo] = {
  ...
  override def next(): (BlockId, InputStream) = {
    ...
    var result: PegasusFetchResult = null
    ...
    while (result == null) {
      ...
      result match {
        case r @SuccessPegasusFetchResult(
        blockId, mapIndex, address, size, buf, isNetworkReqDone) =>
          ...
          val in = try {
            ...
            case e: IOException =>
              // Actually here we know the buf is a PegasusShuffleOutputManagedBuffer
              logError("Failed to create input stream from block backed by Pegasus shuffle output", e)
          ...
          try {
            ...
            case e: IOException =>
              if (corruptedBlocks.contains(blockId)) {
                ...
                fetchRequests += PegasusFetchRequest(address, Array(FetchBlockInfo(blockId, size, mapIndex)))
                result = null
              }
          } finally {
            if (input == null) {
        ...
        case FailurePegasusFetchResult(blockId, mapIndex, address, e) =>
          throwFetchFailedException(blockId, mapIndex, address, e)
      }
    ...
    currentResult = result.asInstanceOf[SuccessPegasusFetchResult]
  ...
  private def fetchUpToMaxBytes(): Unit = {
    ...
    while (isRemoteBlockFetchable(fetchRequests)) {
      ...
      if (isRemoteAddressMaxedOut(remoteAddress, request)) {
        ...
        val defReqQueue =
          deferredFetchRequests.getOrElse(remoteAddress, new Queue[PegasusFetchRequest]())
  ...
  private def send(remoteAddress: BlockManagerId, request: PegasusFetchRequest): Unit = {
    sendRequest(request)
    numBlocksInFlightPerAddress(remoteAddress) =
      numBlocksInFlightPerAddress.getOrElse(remoteAddress, 0) + request.blocks.size
  }

  private def isRemoteBlockFetchable(fetchReqQueue: Queue[PegasusFetchRequest]): Boolean = {
  ...
  private def isRemoteAddressMaxedOut(
      remoteAddress: BlockManagerId, request: PegasusFetchRequest): Boolean = {
    numBlocksInFlightPerAddress.getOrElse(remoteAddress, 0) + request.blocks.size >
      maxBlocksInFlightPerAddress
  }


  private[pegasus] def throwFetchFailedException(
...
private class BufferReleasingInputStream(
    ...
    private[pegasus] val delegate: InputStream,
    private val iterator: PegasusShuffleBlockIterator,
 ...
 */
private class PegasusShuffleCompletionListener(var data: PegasusShuffleBlockIterator)
...
private[pegasus] object PegasusShuffleBlockIterator {

  private val maxConcurrentFetches =
    PegasusShuffleManager.getConf.get(PegasusShuffleConf.NUM_CONCURRENT_FETCH)
  private val eagerRequirement =
    PegasusShuffleManager.getConf.get(PegasusShuffleConf.DATA_FETCH_EAGER_REQUIREMENT)
   ...
   */
  private[pegasus] case class FetchBlockInfo(blockId: BlockId, size: Long, mapIndex: Int)
   ...
   */
  case class PegasusFetchRequest(address: BlockManagerId, blocks: Seq[FetchBlockInfo]) {
   ...
   */
  private[pegasus] sealed trait PegasusFetchResult {
    val blockId: BlockId
  }
   ...
   */
  private[pegasus] case class SuccessPegasusFetchResult(
      blockId: BlockId,
      mapIndex: Int,
      address: BlockManagerId,
      size: Long,
      buf: ManagedBuffer,
      isNetworkReqDone: Boolean) extends PegasusFetchResult {
    require(buf != null)
    require(size >= 0)
  }
   ...
   */
  private[pegasus] case class FailurePegasusFetchResult(
      blockId: BlockId,
      mapIndex: Int,
      address: BlockManagerId,
      e: Throwable) extends PegasusFetchResult
