...
import org.apache.spark.util.collection.PegasusAppendOnlyMap
...
/**
  ...
  */
@DeveloperApi
class PegasusAggregator[K, V, C](agg: Aggregator[K, V, C], resolver: PegasusShuffleBlockResolver)
    extends Aggregator[K, V, C](agg.createCombiner, agg.mergeValue, agg.mergeCombiners) {

  override def combineValuesByKey(
      iter: Iterator[_ <: Product2[K, V]],
      context: TaskContext): Iterator[(K, C)] = {
    val combiners = new PegasusAppendOnlyMap[K, V, C](
  ...
  override def combineCombinersByKey(
      ...
      context: TaskContext): Iterator[(K, C)] = {
    val combiners = new PegasusAppendOnlyMap[K, C, C](
  ...
  private def updateMetrics(context: TaskContext, map: PegasusAppendOnlyMap[_, _, _]): Unit = {
