...
import org.apache.spark.shuffle.pegasus._
import org.apache.spark.shuffle.pegasus.storage._
import org.apache.spark.storage.BlockId
import org.apache.spark.util.CompletionIterator
import org.apache.spark.util.collection.PegasusAppendOnlyMap.HashComparator
...
/**
  ...
  */
@DeveloperApi
class PegasusAppendOnlyMap[K, V, C](
    createCombiner: V => C,
    mergeValue: (C, V) => C,
  ...
  : DiskMapIterator = {
    val (blockId, path) = resolver.createTempLocalBlock()
    val writer: PegasusBlockObjectWriter = PegasusShuffleUtils.getPegasusWriter(
      blockId, path, serializerManager, ser, fileBufferSize, writeMetrics)
    ...
    new DiskMapIterator(path, blockId, batchSizes)
  }
  ...
  override def iterator: Iterator[(K, C)] = {
    if (currentMap == null) {
      throw new IllegalStateException(
        "PegasusAppendOnlyMap.iterator is destructive and should only be called once.")
    }
  ...
  /**
    ...
    */
  private class DiskMapIterator(path: PegasusPath, blockId: BlockId, batchSizes: ArrayBuffer[Long])
      extends Iterator[(K, C)]
  {
    private val batchOffsets = batchSizes.scanLeft(0L)(_ + _)  // Size will be batchSize.length + 1
    // TO DO: remove the assert here?
    assert(ps.getPartitionStatus(path).getLen == batchOffsets.last,
      "Partition length is not equal to the last batch offset:\n" +
          s"    file length = ${ps.getPartitionStatus(path).getLen}\n" +
          s"    last batch offset = ${batchOffsets.last}\n" +
          s"    all batch offsets = ${batchOffsets.mkString(",")}"
    )

    private var batchIndex = 0  // Which batch we're in
    private var dataStream: PegasusDataInputStream = null
    ...
    private def nextBatchStream(): DeserializationStream = {
      ...
      if (batchIndex < batchOffsets.length - 1) {
        ...
        dataStream = ps.openPartition(path, batchIndex, batchIndex+1)
        batchIndex += 1
  ...
  private def hashKey(kc: (K, C)): Int = PegasusAppendOnlyMap.hash(kc._1)
...
private[spark] object PegasusAppendOnlyMap {
