...
/**
  ...
  */
private[spark] class PegasusShuffleReader[K, C](
...
extends ShuffleReader[K, C] with Logging {
  ...
  override def read(): Iterator[Product2[K, C]] = {
    val wrappedStreams = new PegasusShuffleBlockIterator(
      context,
      resolver,
      blocksByAddress,
    ...
    val aggregatedIter: Iterator[Product2[K, C]] = if (dep.aggregator.isDefined) {
      val pegasusAggregator = dep.aggregator.map(new PegasusAggregator(_, resolver))
      if (dep.mapSideCombine) {
        // We are reading values that are already combined
        val combinedKeyValuesIterator = interruptibleIter.asInstanceOf[Iterator[(K, C)]]
        pegasusAggregator.get.combineCombinersByKey(combinedKeyValuesIterator, context)
      } else {
        // We don't know the value type, but also don't care -- the dependency *should*
        // have made sure its compatible w/ this aggregator, which will convert the value
        // type to the combined type C
        val keyValuesIterator = interruptibleIter.asInstanceOf[Iterator[(K, Nothing)]]
        pegasusAggregator.get.combineValuesByKey(keyValuesIterator, context)
      }
    } else {
    ...
    val resultIter = dep.keyOrdering match {
      case Some(keyOrd: Ordering[K]) =>
        ...
        val sorter = new PegasusSorter[K, C, C](
