...
//
// gscoped_ptr<int, pegasus::FreeDeleter> foo_ptr(
...
template <typename T> struct IsNotRefCounted {
  enum {
    value = !base::is_convertible<T*, pegasus::subtle::RefCountedBase*>::value &&
        !base::is_convertible<T*, pegasus::subtle::RefCountedThreadSafeBase*>::
            value
  };
};
...
// types.
template <class T, class D = pegasus::DefaultDeleter<T> >
class gscoped_ptr {
  MOVE_ONLY_TYPE_FOR_CPP_03(gscoped_ptr, RValue)

  COMPILE_ASSERT(pegasus::internal::IsNotRefCounted<T>::value,
 ...
 private:
  typedef pegasus::internal::gscoped_ptr_impl<element_type, deleter_type>
 ...
 private:
  ...
  pegasus::internal::gscoped_ptr_impl<element_type, deleter_type> impl_;
...
// DEPRECATED: Use gscoped_ptr<C, pegasus::FreeDeleter> instead.
//
// gscoped_ptr_malloc<> is similar to gscoped_ptr<>, but it accepts a
// second template argument, the functor used to free the object.

template<class C, class FreeProc = pegasus::FreeDeleter>
