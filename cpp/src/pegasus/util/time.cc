...
void pegasus::SleepForMs(const int64_t duration_ms) {
...
string pegasus::ToStringFromUnix(int64_t s, TimePrecision p) {
  chrono::system_clock::time_point t = TimepointFromUnix(s);
  return ToString(t, p, false);
}

string pegasus::ToUtcStringFromUnix(int64_t s, TimePrecision p) {
  chrono::system_clock::time_point t = TimepointFromUnix(s);
  return ToString(t, p, true);
}

string pegasus::ToStringFromUnixMillis(int64_t ms, TimePrecision p) {
  chrono::system_clock::time_point t = TimepointFromUnixMillis(ms);
  return ToString(t, p, false);
}

string pegasus::ToUtcStringFromUnixMillis(int64_t ms, TimePrecision p) {
  chrono::system_clock::time_point t = TimepointFromUnixMillis(ms);
  return ToString(t, p, true);
}

string pegasus::ToStringFromUnixMicros(int64_t us, TimePrecision p) {
  chrono::system_clock::time_point t = TimepointFromUnixMicros(us);
  return ToString(t, p, false);
}

string pegasus::ToUtcStringFromUnixMicros(int64_t us, TimePrecision p) {
  chrono::system_clock::time_point t = TimepointFromUnixMicros(us);
  return ToString(t, p, true);
}

/*
string pegasus::ToStringFromUnixMicros(int64_t us, const Timezone& tz,
    TimePrecision p) {
  chrono::system_clock::time_point t = TimepointFromUnixMicros(us);
  const char* fmt;
  switch (p) {
