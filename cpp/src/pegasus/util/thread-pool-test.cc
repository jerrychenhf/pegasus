...
TEST(ThreadPoolTest, SynchronousThreadPoolTimeouts) {
  ...
  ASSERT_EQ(timeout_status.code(), StatusCode::ThreadPoolTaskTimeoutError);
  ...
  ASSERT_EQ(queued_task_status.code(), StatusCode::ThreadPoolTaskTimeoutError);
  ...
  ASSERT_EQ(fail_to_submit_status.code(), StatusCode::ThreadPoolSubmitFailed);
