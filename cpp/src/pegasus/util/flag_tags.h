...
// a global registry. PEGASUS uses the following flag tags:
//
// - "stable":
...
//
//  DEFINE_bool(sometimes_crash, false, "This flag makes PEGASUS crash a lot");
...
#ifndef PEGASUS_UTIL_FLAG_TAGS_H
#define PEGASUS_UTIL_FLAG_TAGS_H
...
#define TAG_FLAG(flag_name, tag) \
  namespace { \
    ::pegasus::flag_tags_internal::FlagTagger t_##flag_name##_##tag( \
        AS_STRING(flag_name), AS_STRING(tag)); \
  } \
  COMPILE_ASSERT(sizeof(decltype(FLAGS_##flag_name)), flag_does_not_exist); \
  COMPILE_ASSERT(sizeof(::pegasus::FlagTags::tag), invalid_tag)
...
#endif /* PEGASUS_UTIL_FLAG_TAGS_H */
