...
#ifdef PEGASUS_WITH_BACKTRACE
#include <execinfo.h>
#endif
#include <cstdlib>
#include <iostream>

#ifdef PEGASUS_USE_GLOG
#include <signal.h>
#include <vector>
...
// This is the default implementation of pegasus log,
// which is independent of any libs.
class CerrLog {
 public:
  explicit CerrLog(PegasusLogLevel severity) : severity_(severity), has_logged_(false) {}

  virtual ~CerrLog() {
    if (has_logged_) {
      std::cerr << std::endl;
    }
    if (severity_ == PegasusLogLevel::PEGASUS_FATAL) {
      PrintBackTrace();
      std::abort();
    }
  }
  ...
  CerrLog& operator<<(const T& t) {
    if (severity_ != PegasusLogLevel::PEGASUS_DEBUG) {
      has_logged_ = true;
      std::cerr << t;
    }
    return *this;
 ...
 protected:
  const PegasusLogLevel severity_;
  bool has_logged_;
...
PegasusLogLevel PegasusLog::severity_threshold_ = PegasusLogLevel::PEGASUS_INFO;
...
// Glog's severity map.
static int GetMappedSeverity(PegasusLogLevel severity) {
  switch (severity) {
    case PegasusLogLevel::PEGASUS_DEBUG:
      return google::GLOG_INFO;
    case PegasusLogLevel::PEGASUS_INFO:
      return google::GLOG_INFO;
    case PegasusLogLevel::PEGASUS_WARNING:
      return google::GLOG_WARNING;
    case PegasusLogLevel::PEGASUS_ERROR:
      return google::GLOG_ERROR;
    case PegasusLogLevel::PEGASUS_FATAL:
      return google::GLOG_FATAL;
    default:
      PEGASUS_LOG(FATAL) << "Unsupported logging level: " << static_cast<int>(severity);
...
void PegasusLog::StartPegasusLog(const std::string& app_name,
                             PegasusLogLevel severity_threshold,
  ...
  if (!log_dir.empty()) {
    ...
    for (int i = static_cast<int>(severity_threshold_);
         i <= static_cast<int>(PegasusLogLevel::PEGASUS_FATAL); ++i) {
      int level = GetMappedSeverity(static_cast<PegasusLogLevel>(i));
...
void PegasusLog::UninstallSignalAction() {
#ifdef PEGASUS_USE_GLOG
  PEGASUS_LOG(DEBUG) << "Uninstall signal handlers.";
  ...
  for (int signal_num : installed_signals) {
    PEGASUS_CHECK(signal(signal_num, SIG_DFL) != SIG_ERR);
  }
#else
  ...
  for (int signal_num : installed_signals) {
    PEGASUS_CHECK(sigaction(signal_num, &sig_action, NULL) == 0);
  }
#endif
#endif
}

void PegasusLog::ShutDownPegasusLog() {
#ifdef PEGASUS_USE_GLOG
  if (!log_dir_->empty()) {
...
void PegasusLog::InstallFailureSignalHandler() {
#ifdef PEGASUS_USE_GLOG
  google::InstallFailureSignalHandler();
#endif
}

bool PegasusLog::IsLevelEnabled(PegasusLogLevel log_level) {
  return log_level >= severity_threshold_;
}

PegasusLog::PegasusLog(const char* file_name, int line_number, PegasusLogLevel severity)
...
bool PegasusLog::IsEnabled() const { return is_enabled_; }

PegasusLog::~PegasusLog() {
