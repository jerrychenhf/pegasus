...
#ifndef PEGASUS_TESTUTIL_GTEST_UTIL_H_
#define PEGASUS_TESTUTIL_GTEST_UTIL_H_
...
#define EXPECT_OK(status)                                          \
  do {                                                             \
    ...
    EXPECT_TRUE(status_.ok()) << "Error: " << status_.ToString(); \
  } while (0)

#define ASSERT_OK(status)                                          \
  do {                                                             \
    const Status& status_ = (status);                              \
    ASSERT_TRUE(status_.ok()) << "Error: " << status_.ToString(); \
  } while (0)
...
#define EXPECT_ERROR(status, err)                                       \
  do {                                                                  \
    const Status& status_ = (status);                                   \
    EXPECT_EQ(status_.code(), err) << "Error: " << status_.ToString(); \
  } while (0)
...
#define ASSERT_ERROR_MSG(status, err)                                        \
  do {                                                                       \
    ...
    ASSERT_EQ(status_.msg().msg(), err) << "Error: " << status_.ToString(); \
  } while (0)

// Basic main() function to be used in gtest unit tests.
...
#endif // PEGASUS_TESTUTIL_GTEST_UTIL_H_
