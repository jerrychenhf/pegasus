...
int BindIpcSock(const std::string& pathname, bool shall_listen) {
  ...
  if (socket_fd < 0) {
    LOG(ERROR) << "socket() failed for pathname " << pathname;
    return -1;
  }
  ...
  if (setsockopt(socket_fd, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<char*>(&on),
                 sizeof(on)) < 0) {
    LOG(ERROR) << "setsockopt failed for pathname " << pathname;
    close(socket_fd);
    return -1;
  }
  ...
  if (pathname.size() + 1 > sizeof(socket_address.sun_path)) {
    LOG(ERROR) << "Socket pathname is too long.";
    close(socket_fd);
    return -1;
  }
  ...
  if (bind(socket_fd, reinterpret_cast<struct sockaddr*>(&socket_address),
    ...
    LOG(ERROR) << "Bind failed for pathname " << pathname;
    close(socket_fd);
    return -1;
  }
  if (shall_listen && listen(socket_fd, 128) == -1) {
    LOG(ERROR) << "Could not listen to socket " << pathname;
    close(socket_fd);
    return -1;
  }
  return socket_fd;
...
Status ConnectIpcSocketRetry(const std::string& pathname, int num_retries,
  ...
  *fd = ConnectIpcSock(pathname);
  while (*fd < 0 && num_retries > 0) {
    LOG(ERROR) << "Connection to IPC socket failed for pathname " << pathname
...
std::unique_ptr<uint8_t[]> ReadMessageAsync(int sock) {
  ...
  if (!s.ok()) {
    ...
    LOG(ERROR) << "Socket has been closed, or some other error has occurred.";
    close(sock);
    return NULL;
  }
...
Status WriteMessage(int fd, MessageType type, int64_t length, uint8_t* bytes) {
  ...
  RETURN_IF_ERROR(WriteBytes(fd, reinterpret_cast<uint8_t*>(&version), sizeof(version)));
  RETURN_IF_ERROR(WriteBytes(fd, reinterpret_cast<uint8_t*>(&type), sizeof(type)));
  RETURN_IF_ERROR(WriteBytes(fd, reinterpret_cast<uint8_t*>(&length), sizeof(length)));
  return WriteBytes(fd, bytes, length * sizeof(char));
}

Status ReadMessage(int fd, MessageType* type, std::vector<uint8_t>* buffer) {
  int64_t version;
  RETURN_IF_ERROR_ELSE(ReadBytes(fd, reinterpret_cast<uint8_t*>(&version), sizeof(version)),
                     *type = MessageType::DisconnectClient);
  CHECK(version == kProtocolVersion) << "version = " << version;
  RETURN_IF_ERROR_ELSE(ReadBytes(fd, reinterpret_cast<uint8_t*>(type), sizeof(*type)),
                     *type = MessageType::DisconnectClient);
  int64_t length_temp;
  RETURN_IF_ERROR_ELSE(
  ...
  RETURN_IF_ERROR_ELSE(ReadBytes(fd, buffer->data(), length),
                     *type = MessageType::DisconnectClient);
  return Status::OK();
}
