...
static void ReleaseBuffer(void* buf_ptr) {
  delete reinterpret_cast<std::shared_ptr<arrow::Buffer>*>(buf_ptr);
}

// Initialize gRPC Slice from arrow Buffer
grpc::Slice SliceFromBuffer(const std::shared_ptr<arrow::Buffer>& buf) {
  // Allocate persistent shared_ptr to control Buffer lifetime
  auto ptr = new std::shared_ptr<arrow::Buffer>(buf);
...
grpc::Status FlightDataSerialize(const FlightPayload& msg, ByteBuffer* out,
  ...
  if (msg.descriptor != nullptr) {
    if (msg.descriptor->size() > kInt32Max) {
      return ToGrpcStatus(arrow::Status::CapacityError("Descriptor size overflow (>= 2**31)"));
    }
  ...
  for (const auto& buffer : ipc_msg.body_buffers) {
    ...
    body_size += static_cast<size_t>(arrow::BitUtil::RoundUpToMultipleOf8(buffer->size()));
  }

  bool has_body = arrow::ipc::Message::HasBody(ipc_msg.type);
  ...
  if (has_body) {
    ...
    for (const auto& buffer : ipc_msg.body_buffers) {
      ...
      const auto remainder = static_cast<int>(
          arrow::BitUtil::RoundUpToMultipleOf8(buffer->size()) - buffer->size());
      if (remainder) {
  ...
  while (pb_stream.BytesUntilTotalBytesLimit()) {
    ...
    switch (field_number) {
      case pb::FlightData::kFlightDescriptorFieldNumber: {
        ...
        pegasus::rpc::FlightDescriptor descriptor;
        GRPC_RETURN_NOT_OK(
            pegasus::rpc::internal::FromProto(pb_descriptor, &descriptor));
        out->descriptor.reset(new pegasus::rpc::FlightDescriptor(descriptor));
      } break;
...
arrow::Status FlightData::OpenMessage(std::unique_ptr<arrow::ipc::Message>* message) {
  return arrow::ipc::Message::Open(metadata, body, message);
}
