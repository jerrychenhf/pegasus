...
class PEGASUS_RPC_EXPORT ServerAuthReader {
 public:
  virtual ~ServerAuthReader() = default;
  virtual arrow::Status Read(std::string* token) = 0;
};

/// \brief A writer for messages to the client during an
/// authentication handshake.
class PEGASUS_RPC_EXPORT ServerAuthSender {
 public:
  virtual ~ServerAuthSender() = default;
  virtual arrow::Status Write(const std::string& message) = 0;
};
...
class PEGASUS_RPC_EXPORT ServerAuthHandler {
 public:
  ...
  virtual arrow::Status Authenticate(ServerAuthSender* outgoing, ServerAuthReader* incoming) = 0;
  ...
  /// \return arrow::Status OK if the token is valid, any other arrow::Status if
  /// validation failed
  virtual arrow::Status IsValid(const std::string& token, std::string* peer_identity) = 0;
};

/// \brief An authentication mechanism that does nothing.
class PEGASUS_RPC_EXPORT NoOpAuthHandler : public ServerAuthHandler {
 public:
