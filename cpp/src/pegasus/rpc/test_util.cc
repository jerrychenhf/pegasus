...
arrow::Status ResolveCurrentExecutable(fs::path* out) {
  ...
  *out = fs::canonical("/proc/self/exe", ec);
  ...
  if (_NSGetExecutablePath(buf, &bufsize) < 0) {
    return arrow::Status::Invalid("Can't resolve current exe: path too large");
  }
  *out = fs::canonical(buf, ec);
#elif defined(_WIN32)
  char buf[MAX_PATH + 1];
  if (!GetModuleFileNameA(NULL, buf, sizeof(buf))) {
    return arrow::Status::Invalid("Can't get executable file path");
  }
  *out = fs::canonical(buf, ec);
#else
  ARROW_UNUSED(ec);
  return arrow::Status::NotImplemented("Not available on this system");
#endif
  if (ec) {
    // XXX fold this into the arrow::Status class?
    return arrow::Status::IOError("Can't resolve current exe: ", ec.message());
  } else {
...
void TestServer::Start() {
  ...
  fs::path current_exe;
  arrow::Status st = ResolveCurrentExecutable(&current_exe);
  if (st.ok()) {
...
class FlightTestServer : public FlightServerBase {
  ...
  arrow::Status RunAction1(const Action& action, std::unique_ptr<ResultStream>* out) {
    std::vector<Result> results;
    for (int i = 0; i < 3; ++i) {
      Result result;
      std::string value = action.body->ToString() + "-part" + std::to_string(i);
      RETURN_NOT_OK(arrow::Buffer::FromString(value, &result.body));
  ...
  arrow::Status RunAction2(std::unique_ptr<ResultStream>* out) {
    // Empty
  ...
  arrow::Status DoAction(const ServerCallContext& context, const Action& action,
    ...
    if (action.type == "action1") {
      ...
      return arrow::Status::NotImplemented(action.type);
    }
  }

  arrow::Status ListActions(const ServerCallContext& context,
  ...
  arrow::Status GetSchema(const ServerCallContext& context, const FlightDescriptor& request,
...
std::shared_ptr<arrow::Schema> NumberingStream::schema() { return stream_->schema(); }

arrow::Status NumberingStream::GetSchemaPayload(FlightPayload* payload) {
  return stream_->GetSchemaPayload(payload);
}

arrow::Status NumberingStream::Next(FlightPayload* payload) {
  RETURN_NOT_OK(stream_->Next(payload));
  if (payload && payload->ipc_message.type == arrow::ipc::Message::RECORD_BATCH) {
    payload->app_metadata = arrow::Buffer::FromString(std::to_string(counter_));
    counter_++;
  }
  return arrow::Status::OK();
}
...
std::shared_ptr<arrow::Schema> ExampleStringSchema() {
  ...
  auto f1 = field("f1", arrow::binary());
  return arrow::schema({f0, f1});
}

std::shared_ptr<arrow::Schema> ExampleDictSchema() {
  std::shared_ptr<arrow::RecordBatch> batch;
  ABORT_NOT_OK(arrow::ipc::test::MakeDictionary(&batch));
  return batch->schema();
}
...
std::vector<FlightInfo> ExampleFlightInfo() {
  ...
  EXPECT_OK(MakeFlightInfo(*schema3, descr3, {endpoint4}, -1, -1, &flight3));
...
arrow::Status ExampleIntBatches(BatchVector* out) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    RETURN_NOT_OK(arrow::ipc::test::MakeIntBatchSized(10 + i, &batch, i));
    out->push_back(batch);
  }
  return arrow::Status::OK();
}
...
arrow::Status ExampleDictBatches(BatchVector* out) {
  ...
  std::shared_ptr<arrow::RecordBatch> batch;
  for (int i = 0; i < 3; ++i) {
    RETURN_NOT_OK(arrow::ipc::test::MakeDictionary(&batch));
    out->push_back(batch);
  }
  return arrow::Status::OK();
}
...
arrow::Status TestServerAuthHandler::Authenticate(ServerAuthSender* outgoing,
...
arrow::Status TestServerAuthHandler::IsValid(const std::string& token,
...
Status GetTestResourceRoot(std::string* out) {
  const char* c_root = std::getenv("PEGASUS_TEST_DATA");
  if (!c_root) {
    return arrow::Status::IOError("Test resources not found, set PEGASUS_TEST_DATA");
  }
  ...
  *out = std::vector<CertKeyPair>();
  for (int i = 0; i < 2; i++) {
    try {
      ...
      cert_path << root << "/rpc/cert" << i << ".pem";
      std::stringstream key_path;
      key_path << root << "/rpc/cert" << i << ".key";
