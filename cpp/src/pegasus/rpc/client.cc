  ...
  void Intercept(grpc::experimental::InterceptorBatchMethods* methods) {
    ...
    if (methods->QueryInterceptionHookPoint(
      ...
      for (const auto& entry : *methods->GetRecvInitialMetadata()) {
        headers.insert({arrow::util::string_view(entry.first.data(), entry.first.length()),
...
class GrpcClientInterceptorAdapterFactory
 ...
 public:
  ...
  grpc::experimental::Interceptor* CreateClientInterceptor(
    ...
    arrow::util::string_view method(info->method());
...
class GrpcIpcMessageReader : public arrow::ipc::MessageReader {
 public:
  ...
  Status ReadNextMessage(std::unique_ptr<arrow::ipc::Message>* out) override {
    if (stream_finished_) {
      *out = nullptr;
...
Status GrpcStreamReader::Open(std::unique_ptr<ClientRpc> rpc,
  ...
  *out = std::unique_ptr<GrpcStreamReader>(new GrpcStreamReader);
  ...
  return arrow::ipc::RecordBatchStreamReader::Open(std::move(message_reader),
                                            &(*out)->batch_reader_);
}
...
class GrpcStreamWriter : public FlightStreamWriter {
 public:
  ...
  static Status Open(
      const FlightDescriptor& descriptor, const std::shared_ptr<arrow::Schema>& schema,
  ...
  Status WriteWithMetadata(const arrow::RecordBatch& batch,
                           std::shared_ptr<arrow::Buffer> app_metadata) override {
  ...
  void set_memory_pool(arrow::MemoryPool* pool) override {
 ...
 private:
  ...
  std::unique_ptr<arrow::ipc::RecordBatchWriter> batch_writer_;
...
class DoPutPayloadWriter : public arrow::ipc::internal::IpcPayloadWriter {
 public:
  DoPutPayloadWriter(
  ...
  Status WritePayload(const arrow::ipc::internal::IpcPayload& ipc_payload) override {
    FlightPayload payload;
    payload.ipc_message = ipc_payload;

    if (first_payload_) {
      // First Flight message needs to encore the Flight descriptor
      if (ipc_payload.type != arrow::ipc::Message::SCHEMA) {
      ...
      RETURN_NOT_OK(arrow::Buffer::FromString(str_descr, &payload.descriptor));
      first_payload_ = false;
    } else if (ipc_payload.type == arrow::ipc::Message::RECORD_BATCH &&
...
Status GrpcStreamWriter::Open(
  ...
  std::unique_ptr<arrow::ipc::internal::IpcPayloadWriter> payload_writer(new DoPutPayloadWriter(
      descriptor, std::move(rpc), std::move(response), read_mutex, writer, result.get()));
  RETURN_NOT_OK(arrow::ipc::internal::OpenRecordBatchWriter(std::move(payload_writer), schema,
...
class GrpcMetadataReader : public FlightMetadataReader {
 public:
  ...
  Status ReadMetadata(std::shared_ptr<arrow::Buffer>* out) override {
    std::lock_guard<std::mutex> guard(*read_mutex_);
    pb::PutResult message;
    if (reader_->Read(&message)) {
      *out = arrow::Buffer::FromString(std::move(*message.mutable_app_metadata()));
    } else {
      // Stream finished
      *out = nullptr;
    }
